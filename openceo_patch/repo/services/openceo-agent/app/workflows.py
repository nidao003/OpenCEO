...
from .prompts import (MONTHLY_SUMMARY_SYSTEM, PPT_OUTLINE_SYSTEM, REPORT_ANALYST_SYSTEM,
                      VIDEO_NARRATION_SYSTEM, WEEKLY_SUMMARY_SYSTEM)
...
async def ingest_report(
    ...
    if external_provider and external_user_id:
        identity = await rpc.call("People", "upsertIdentity", {
    ...
    structured: dict[str, Any] = {"deterministic_items": [i.model_dump() for i in report.items]}
    if llm.enabled:
    ...
    report_id = await rpc.call("Reports", "ingest", {
    ...
    for item in report.items:
        ...
        if raw_project:
            ...
            if match.get("status") == "unmatched":
                candidate_id = await rpc.call("Projects", "createCandidate", {
                    "name": raw_project,
                    "confidence": 0.65,
        ...
        await rpc.call("Reports", "addItem", {
            "reportId": report_id,
        ...
        if project_id or candidate_id:
            await rpc.call("Projects", "addEvent", {
    ...
    # Add one observation per matched canonical project. Unknown projects wait for
    # human candidate resolution before becoming authoritative project state.
        ...
        if llm.enabled and structured.get("llm"):
            for candidate in structured["llm"].get("project_observations", []):
                if candidate.get("raw_project_name") == raw_project:
                    observation.update({k: v for k, v in candidate.items() if k in observation})
                    observation["confidence"] = float(candidate.get("confidence") or 0.7)
                    break
        await rpc.call("Projects", "addObservation", {
            "projectId": project_id,
            "reportId": report_id,
            **observation,
        })
...
async def generate_output(output_type: str, period_key: str = "") -> dict[str, Any]:
    ...
    if output_type in {"weekly_summary", "meeting_agenda"}:
        weekly_reports = await rpc.call("Reports", "listReports", {
            "reportType": "weekly",
            "reportBatchKey": period_key,
            "limit": 200,
        })
        report_context["weekly_reports"] = _compact_reports(weekly_reports)
    elif output_type == "monthly_summary" and re.fullmatch(r"\d{4}-\d{2}", period_key or ""):
        ...
        monthly_reports = await rpc.call("Reports", "listReports", {
            "reportType": "monthly",
            "limit": 200,
            "periodStart": start,
            "periodEnd": end,
    ...
    if output_type in ("weekly_summary", "monthly_summary", "meeting_agenda"):
        system = WEEKLY_SUMMARY_SYSTEM if output_type != "monthly_summary" else MONTHLY_SUMMARY_SYSTEM
        if llm.enabled:
            content = await llm.complete_text(system, json.dumps({
                "company": profile,
                "state": state,
        ...
        source_type = "monthly_summary" if re.fullmatch(r"\d{4}-\d{2}", period_key or "") else "weekly_summary"
        latest = await rpc.call("Outputs", "latest", {"outputType": source_type, "periodKey": period_key})
        source = latest.get("content") or deterministic_summary(state, title_map.get(source_type, source_type))
        system = PPT_OUTLINE_SYSTEM if output_type == "ppt_outline" else VIDEO_NARRATION_SYSTEM
        if llm.enabled:
    ...
    saved = await rpc.call("Outputs", "save", {
        "outputType": output_type,
        "content": content,
        "periodKey": period_key,
        "status": "draft",
        "metadata": {"llm_enabled": llm.enabled},
    })
